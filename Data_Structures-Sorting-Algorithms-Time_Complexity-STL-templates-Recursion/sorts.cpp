...
void sortUsingPtrs(vector<Sensor>& sensors, bool comp(const Sensor*, const Sensor*))
{
    ...
    std::sort(auxSensorPointers.begin(), auxSensorPointers.end(), comp);
    ...
    //        order.
    // take the first pointer in AuxSensorPointers, find the position of what it is pointing to in the AuxSensors array. Set that sensor to be the first element in the sensors array
    for (unsigned int i = 0; i < auxSensorPointers.size(); i++)
    {
        for (unsigned int k = 0; k < auxSensors.size(); k++)
        {
            if (&auxSensors[k] == auxSensorPointers[i])
            {
                sensors[i] = auxSensors[k];
                break;
            }
        }
    }
